...
class UrlQuery {
    ...
    sort() {
        this._properties.queryParams = this._properties.queryParams.sort((a, b) => {
            let n = CJU.compareStrings(a.name, b.name);
...
exports.PathNameCollection = PathNameCollection;
...
class UriBuilderOld {
    ...
    get username() { return (CJU.notNil(this._properties.userName) || CJU.notString(this._properties.password)) ? this._properties.userName : ''; }
    ...
    get hash() { return (CJU.notNilOrEmpty(this._properties.fragment)) ? '#' + this._properties.fragment : ''; }
    set hash(value) {
        if (CJU.notNilOrEmpty(value)) {
            if (value.startsWith('#'))
                value = value.substr(1);
